...
from art_datastructure.datastructure import GlobalTree, DataElement, DataObject, assemble_tree
...
class Read3dmFile():
    ...
    def __convert_rhino_obj_to_shapely_obj(self, obj:rhino3dm._rhino3dm.ObjectType)->None or Point or LineString or Polygon:
        ...
        # 把TextDot转换成点，渲染的时候把文字渲染到对应的位置上
            ...
            shapely_geometry = Point((rhino_geometry.GetBoundingBox().Center.X, rhino_geometry.GetBoundingBox().Center.Y))
            return shapely_geometry
    ...
    def __data_structure_processor(self, raw_data:list, groups_info:dict, text_info:dict, height_info:dict, floor_info:dict)->List[DataObject]:
        # 先组装DataElement
        data_elements = []
        for obj in raw_data:
    ...
    def read_3dm_file(self):
        ...
        if READ_OPTION == 0:     # 使用我们的数据结构打包
            return self.__data_structure_processor(raw_data=raw_data, groups_info=groups_info, text_info=text_info, height_info=height_info, floor_info=floor_info)
        else:
            return raw_data
...
if __name__=="__main__":
    ...
    data_objects = Read3dmFile(TEST_FILE_PATH).read_3dm_file()
